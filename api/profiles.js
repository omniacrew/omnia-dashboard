...
export default async function handler(req, res) {
  ...
  // Fetch profiles in the Book Intro list with recent activity
  const filter = `greater-than(updated,${cutoff})`;
  const path = `/api/lists/${listId}/profiles?filter=${encodeURIComponent(filter)}&page[size]=100&sort=-updated`;
